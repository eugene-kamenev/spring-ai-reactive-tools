...
public class NonThinkingAgentTests extends BaseTests {
	...
	void testToolsCustomYamlFormat() {
		...
		assertInstanceOf(FuncCall.class, payloads.get(0));
		assertInstanceOf(FuncResult.class, payloads.get(1));
		assertInstanceOf(Text.class, payloads.get(2));
	...
	void testToolsCustomJsonFormat() {
		List<Payload> payloads = new ArrayList<>();
		var config = new ToolCallConfig(ToolCallHandler.CUSTOM, SchemaFormat.JSON, null, null, null);
	...
	void testToolsCustomJsonRfcFormat() {
		List<Payload> payloads = new ArrayList<>();
		var config = new ToolCallConfig(ToolCallHandler.CUSTOM, SchemaFormat.JSON_RFC, null, null, null);
		var chatAgent = new BaseAgent(getChatClient(), new ObjectMapper(), config) {
			@Tool
			public Mono<String> executeCommand(@ToolParam String command) {
				return EXECUTE_COMMAND.apply(command);
